...
import { getListEmplayee, getListService, getListStore,updateBookingApi} from '../util/api';
import { isPhoneNumber, isValidDate } from '../util/validate';
import { Booking } from '../models/booking';

const AddBooking = ({onClose,updateBooking}) => {
  const [name, setName] = useState(updateBooking.customerName);
  const [phone, setPhone] = useState(updateBooking.phoneNumber);
  const [date, setDate] = useState(updateBooking.dateBooking);
  const [note, setNote] = useState(updateBooking.note);
  const [isFocus, setIsFocus] = useState(false);
  const [FocusAdd, setFocusAdd] = useState(false)

  const [Id_Store, setId_Store] = useState(updateBooking.id_store._id);
  const [listStore, setlistStore] = useState<Array<{_id: string; name: string}>>([]);

  const [Id_Employee, setId_Employee] = useState(updateBooking.id_employee);
  const [listEmployee, setListEmployee] = useState([]);
  const [isFocusEmployee, setIsFocusEmployee] = useState(false);

  const [listServices, setListServices] = useState(updateBooking.services);
  ...
  useEffect(()=>{
    updateBooking.services?.map(item=>{
      setSelectedServices(selectedServices=>[...selectedServices,item._id])
    })
  },[])
  ...
  const handleUpdateBooking = async() => {
     ...
     if (errors.length > 0) {
       errors.forEach(error => console.log(error));
        ...
        const isSuccess = await updateBookingApi(booking) 
        if(isSuccess){
          console.log('Xửa thành công !')
           onClose()
        } else {
         console.log('Xửa thất bại !')
           onClose()
        }
     }
     setFocusAdd(true)
  ...
  return (
    ...
    {FocusAdd && Id_Employee == null && <Text style={{color:'red',marginStart:24,marginTop:12}}>Vui lòng chọn Nhân viên !</Text>}
      ...
      <View style={styles.actionContainer}>
        ...
        <TouchableOpacity onPress={() => {handleUpdateBooking()}}>
          <Image
            style={styles.actionButton}
